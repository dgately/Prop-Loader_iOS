...
/*!
 * Provides a mechanism for passing the laoder state message to the delegate from the main thread. This method
...
- (void) writeBytesToBEE: (NSData *) data tag: (long) tag {
    ...
    if (!udpSocket) {
        ...
        // Bind to the port.
        NSError *error;
        [udpSocket bindToPort: [Common udpPort] error: &error];
        if (error == nil)
            [udpSocket beginReceiving: &error];
    }
    
    [udpSocket sendData: data toHost: xBee.ipAddr port: [Common udpPort] withTimeout: 0.1 tag: tag];
}
